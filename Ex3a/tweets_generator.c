...
int fill_database(FILE *fp,int words_to_read,MarkovChain *markov_chain)
{
  ...
  while(words_read < words_to_read)
  {
    ...
    while (pch != NULL && words_read<words_to_read)
    {
      ...
      if (prev != NULL)
      {
        add_node_to_frequency_list (prev->data, current->data);
...
int main(int argc,char *argv[])
{
  ...
  markov_chain->database = generate_list (markov_chain);
  ...
  for(int tweet = 1;tweet<=tweets_number;tweet++)
  {
    ...
    generate_tweet(first_node,MAX_LEN_TWEET);
    printf("\n");
  }
  free_database (&markov_chain);
  return  EXIT_SUCCESS;
