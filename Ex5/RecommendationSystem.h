...
{
 private:
  std::unordered_map<sp_movie, std::vector<double>> features_;
  std::pair<sp_movie,double> shared_filter(const User& user, int k);
public:
  ...
  sp_movie string_to_ptr(std::string name);

  static bool sort_func(Movie &m1, Movie &m2);
    /**
	...
	friend std::ostream &operator<<(std::ostream &os,RecommendationSystem &rs);
